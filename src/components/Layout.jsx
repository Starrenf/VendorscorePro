...
export default function Layout({ children }) {
  ...
  const secondaryMenu = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Kroonjuwelen",
        icon: ShieldCheck,
        active: pathname.startsWith("/kroonjuwelen"),
      },
      {
        to: "/ai-register",
        label: "AI-register",
      ...
      ...(isAdmin
        ? [
            ...
            {
              to: "/settings",
              label: "Waardelijsten & instellingen",
  ...
  const [presentationMode, setPresentationMode] = useState(() => {
    try {
      return window.localStorage.getItem("VSP_PRESENTATION_MODE") === "true";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem("VSP_PRESENTATION_MODE", presentationMode ? "true" : "false");
    } catch {
  ...
  return (
    <ToastProvider>
      <div className="relative min-h-screen overflow-x-hidden text-slate-900">
        ...
        >
          <div className="mx-auto grid max-w-[1800px] grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 lg:grid-cols-[minmax(220px,310px)_minmax(0,1fr)_auto] lg:gap-4 lg:px-6">
            <div className="flex min-w-0 items-center gap-3.5 lg:gap-4.5">
              <OrbLogo />
              <div className="min-w-0 max-w-[280px] lg:max-w-[320px]">
                <div className="text-[1.25rem] font-extrabold leading-none tracking-tight text-white xl:text-[1.45rem]">
                  VendorScore Pro
                </div>
                <div className="mt-1 hidden max-w-[240px] text-[9px] font-semibold uppercase leading-[1.2] tracking-[0.14em] text-cyan-100/78 xl:block">
            ...
            <div className="hidden lg:flex min-w-0 items-center justify-center gap-2 xl:gap-3">
              <nav className="flex min-w-0 items-center gap-2 rounded-[2rem] border border-white/10 bg-white/[0.055] px-2.5 py-1.5 shadow-[0_14px_40px_rgba(0,0,0,0.16)] backdrop-blur-md xl:gap-2.5 xl:px-3">
              ...
              <div className="relative">
                ...
                {desktopMoreOpen ? (
                  <div className="absolute right-0 top-[calc(100%+10px)] vsp-header-more-panel w-72 rounded-2xl border border-white/10 bg-[rgba(8,27,55,0.96)] p-2 shadow-2xl backdrop-blur-xl">
            ...
            <div className="hidden lg:flex items-center gap-2.5 justify-self-end">
              <button
                type="button"
                className={("navbtn navbtn-medium" + (presentationMode ? " navbtn-active" : ""))}
                onClick={() => setPresentationMode((v) => !v)}
                title="Presentatiemodus voor groot scherm"
              >
                <MonitorUp className="navbtn-icon" aria-hidden="true" />
        ...
        <main className="vs-main mx-auto w-full max-w-[1920px] px-4 py-6 sm:px-5 lg:px-8 2xl:px-10">{children}</main>

        <footer className="border-t border-slate-200 bg-white/95 backdrop-blur">
          <div className="mx-auto grid max-w-[1920px] gap-4 px-4 py-6 text-sm text-slate-600 md:grid-cols-3 md:items-center lg:px-8 2xl:px-10">
