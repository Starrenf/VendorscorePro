...
export default function SupplierMeetingsTab({ supplier, organization }) {
  ...
  function addRow() {
    if (!supplier?.id || !organizationId) {
      setError("Overleg toevoegen is nog niet mogelijk: leverancier of organisatie ontbreekt.");
      return;
    }
  ...
  async function saveRow(row) {
    ...
    setMessage("");
    if (!client) return;
    if (!supplier?.id || !organizationId) { setError("Overleg opslaan is nog niet mogelijk: leverancier of organisatie ontbreekt."); return; }
  ...
  return (
    <div className="max-w-5xl space-y-4">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <button className="btn btn-primary" onClick={addRow} disabled={!supplier?.id || !organizationId}>+ Overleg</button>
      </div>
