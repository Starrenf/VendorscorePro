...
function displayName(value, currentUserId, currentProfile, currentEmail) {
  if (!value) return "Onbekend";
  if (value === currentUserId) return currentProfile?.full_name || currentEmail || "Jij";
  if (/^[0-9a-f-]{36}$/i.test(String(value))) return "Gebruiker";
  return value;
}
...
export default function ApplicationContractSummary({ supplier, organization, application }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (error) {
        ...
        let updater = data.updated_by_name || data.updated_by || "";
        if (data.updated_by && /^[0-9a-f-]{36}$/i.test(String(data.updated_by))) {
          const { data: prof } = await client.from("profiles").select("full_name").eq("id", data.updated_by).maybeSingle();
          updater = prof?.full_name || data.updated_by;
        }
  ...
  const matches = useMemo(() => {
    ...
    if (!needle) return 0;
    return (stripHtml(summaryText).toLowerCase().match(new RegExp(needle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) || []).length;
  }, [query, summaryText]);
  ...
  async function save() {
    ...
    try {
      ...
      setUpdatedBy(displayName(data?.updated_by || payload.updated_by || "", session?.user?.id, profile, session?.user?.email));
      setStatus("clean");
    ...
    <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
      ...
      <div className="mt-3">
        ...
        <div className="mt-1">
          <RichTextEditor
            value={summaryText}
            onChange={markDirty(setSummaryText)}
            placeholder="Plak of schrijf hier de contractsamenvatting volgens het Gilde-template…"
            minHeight={280}
          />
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between gap-3 flex-wrap">
        <div className="text-xs text-slate-500">
          Laatst bijgewerkt: {formatDate(updatedAt)}{updatedBy ? ` · door ${displayName(updatedBy, session?.user?.id, profile, session?.user?.email)}` : ""}
        </div>
