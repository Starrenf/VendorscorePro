...
export default function SupplierActionsTab({ supplier, organization }) {
  ...
  const organizationId = organization?.id || supplier?.organization_id || null;
  ...
  async function load() {
    if (!client || !supplier?.id || !organizationId) return;
    setLoading(true);
    setError("");
  ...
  useEffect(() => { load(); }, [supplier?.id, organizationId]);
  ...
  function addRow() {
    if (!supplier?.id || !organizationId) {
      setError("Actie toevoegen is nog niet mogelijk: leverancier of organisatie ontbreekt.");
      return;
    }
    setRows((prev) => [emptyRow(organizationId, supplier.id), ...prev]);
  }
  ...
  async function saveRow(row) {
    ...
    setMessage("");
    if (!client) return;
    if (!supplier?.id || !organizationId) { setError("Actie opslaan is nog niet mogelijk: leverancier of organisatie ontbreekt."); return; }
    ...
    try {
      ...
      await saveWithToast(query, toast, {
        loading: "Actie opslaan...",
        success: "Actie opgeslagen.",
        error: "Actie opslaan mislukt.",
      });
      setMessage("Actie opgeslagen.");
      await load();
    } catch (err) {
      setError(err?.message || "Actie opslaan mislukt.");
    }
  }
  ...
  async function deleteRow(row) {
    ...
    if (!ok) return;
    try {
      await saveWithToast(client.from("supplier_actions").delete().eq("id", row.id), toast, {
        loading: "Actie verwijderen...",
        success: "Actie verwijderd.",
        error: "Actie verwijderen mislukt.",
      });
      setMessage("Actie verwijderd.");
      await load();
    } catch (err) {
      setError(err?.message || "Actie verwijderen mislukt.");
    }
  }
    ...
    <div className="max-w-5xl space-y-4">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-600">
          ...
          <button className="btn btn-primary" onClick={addRow} disabled={!supplier?.id || !organizationId}>+ Actie</button>
        </div>
      </div>
        ...
        {rows.map((row, idx) => (
          <div key={row.id || `new-${idx}`} className="card p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button className="btn btn-primary" onClick={() => saveRow(row)} disabled={!supplier?.id || !organizationId}>Opslaan</button>
            </div>
          </div>
        ))}
      </div>
