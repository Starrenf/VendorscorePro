...
import ApplicationContractSummary from "./ApplicationContractSummary";
...
function dedupeApplications(rows) {
  const byKey = new Map();
  for (const row of rows || []) {
    const key = `${row?.supplier_id || ""}::${normalizeAppName(row?.name || row?.application_name || row?.title)}` || row?.id;
...
export default function SupplierApplicationsTab({ supplier, organization }) {
  ...
  async function loadApplications() {
    ...
    if (error) {
      ...
      setApplications(dedupeApplications(data || []));
    }
  ...
  async function saveAssignment(row) {
    ...
    const payload = {
      ...
      application_id: row.application_id || findApplicationByName(row.application_name)?.id || null,
  ...
  async function saveApplication() {
    ...
    const existingApp = findApplicationByName(newApp.name);
    if (existingApp) {
      setOpenApplicationId(existingApp.id);
      setAppEditorOpen(false);
      setError(`Applicatie "${existingApp.name}" bestaat al bij deze leverancier. Open de bestaande applicatie in plaats van een dubbele regel aan te maken.`);
      toast?.info?.(`Applicatie "${existingApp.name}" bestaat al.`, "Dubbele applicatie voorkomen");
      return;
    }
  ...
  const selectedApplication = useMemo(
    () => applications.find((app) => app.id === openApplicationId) || null,
    [applications, openApplicationId],
  );

  const selectedAssignments = useMemo(() => {
    if (!selectedApplication) return [];
    const appName = normalizeAppName(selectedApplication.name);
    return assignments.filter((row) =>
      row.application_id === selectedApplication.id ||
      normalizeAppName(row.application_name) === appName,
    );
  }, [assignments, selectedApplication]);

  function findApplicationByName(name) {
    const normalized = normalizeAppName(name);
    if (!normalized) return null;
    return applications.find((app) => normalizeAppName(app.name) === normalized) || null;
  }
      ...
      <div className="card p-4">
        ...
        {applications.length > 0 ? (
          <div className="mt-3 grid md:grid-cols-2 gap-3">
            {applications.map((app) => {
              const isSelected = openApplicationId === app.id;
              return (
                <button
                  key={app.id}
                  type="button"
                  className={`text-left rounded-xl border bg-white p-3 transition hover:border-sky-300 hover:shadow-sm ${isSelected ? "border-sky-400 ring-2 ring-sky-100" : "border-slate-200"}`}
                  onClick={() => setOpenApplicationId((current) => (current === app.id ? null : app.id))}
                >
                  ...
                  <div className="mt-3 text-xs font-semibold text-sky-700">
                    {isSelected ? "Applicatie geopend" : "Open applicatie-details / contractsamenvatting"}
                  </div>
                </button>
              );
            })}
        ...
        {selectedApplication ? (
          <div className="mt-4 rounded-2xl border border-sky-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                <div className="text-xs uppercase tracking-wide text-sky-700 font-semibold">Applicatie-detail</div>
                <h3 className="text-lg font-semibold text-slate-900 mt-1">{selectedApplication.name}</h3>
                <p className="text-sm text-slate-600 mt-1">
                  Hier leg je de applicatiegegevens en de contractsamenvatting vast. De leverancier blijft de contractpartij; deze applicatie is het product/de dienst.
                </p>
              </div>
              <button className="btn" type="button" onClick={() => setOpenApplicationId(null)}>Sluiten</button>
            </div>

            <div className="mt-4 grid md:grid-cols-3 gap-3">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-xs text-slate-500">Status</div>
                <div className="mt-1 font-semibold">{selectedApplication.is_active === false ? "Inactief" : "Actief"}</div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-xs text-slate-500">Kroonjuweel</div>
                <div className="mt-1 font-semibold">{selectedApplication.is_critical ? "Ja" : "Nee / niet gemarkeerd"}</div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-xs text-slate-500">Functioneel beheer</div>
                <div className="mt-1 font-semibold">{selectedAssignments.length} record(s)</div>
              </div>
            </div>

            {selectedApplication.description ? (
              <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-xs text-slate-500">Omschrijving</div>
                <div className="mt-1 text-sm text-slate-700 whitespace-pre-wrap">{selectedApplication.description}</div>
              </div>
            ) : null}

            <ApplicationContractSummary supplier={supplier} organization={organization} application={selectedApplication} />
          </div>
        ) : applications.length > 0 ? (
          <div className="mt-4 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
            Klik op een applicatiekaart om de applicatie-details en contractsamenvatting te openen.
          </div>
        ) : null}
      </div>
