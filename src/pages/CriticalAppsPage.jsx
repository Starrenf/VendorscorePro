...
export default function CriticalAppsPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setErr("");
      try {
        const [{ data: apps, error: appsErr }, { data: suppliers, error: suppliersErr }, { data: governanceItems, error: govErr }, { data: assignments, error: assignErr }] = await Promise.all([
          client.from("applications").select("id,organization_id,supplier_id,name,description,is_active,is_critical,functional_owner").eq("organization_id", orgId).eq("is_critical", true).eq("is_active", true).order("name", { ascending: true }),
          ...
          client.from("functional_admin_assignments").select("supplier_id,application_id,application_name,contact_name,email,role_title,is_primary").eq("organization_id", orgId),
        ]);
        if (appsErr) throw appsErr;
        if (suppliersErr) throw suppliersErr;
        if (govErr) throw govErr;
        if (assignErr && assignErr.code !== "PGRST205" && assignErr.code !== "42P01") throw assignErr;
