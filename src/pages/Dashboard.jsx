...
export default function Dashboard() {
    ...
    async function run() {
      ...
      try {
        ...
        const { data: suppliers, error: supplierErr } = await client
          .from("suppliers")
          .select("id,name,classification,supplier_type,created_at,category")
  ...
  const summary = useMemo(() => {
    ...
    return { suppliers, avgGovernance, green, amber, red, evaluatedThisYear, openActions, healthyPercent, avgRisk, avgPerformance, trend, coveragePercent, aiSuppliers };
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        ...
        <div className="card p-6">
          ...
          <div className="mt-4 space-y-3 text-sm text-slate-700">
            ...
            <div className="rounded-2xl border border-slate-200 p-4 bg-slate-50">
              <div className="font-semibold">Voorbereid op rapportage</div>
              <div className="mt-1 text-slate-600">Deze drill-down opzet vormt straks de basis voor rapportages per leverancier en managementoverzichten.</div>
            </div>
          </div>
        </div>
      </div>
