...
import { AlertTriangle, Bot, Building2, CheckSquare, ClipboardCheck, Gem, ShieldCheck } from "lucide-react";
...
function MetricCard({ label, value, subtext, tone = "slate", active = false, onClick, icon: Icon }) {
  const tones = {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        {Icon ? <div className="rounded-2xl bg-white/70 p-2 shadow-sm"><Icon size={20} /></div> : null}
      </div>
...
function BarRow({ label, value, total, tone = "bg-slate-500", onClick, active = false }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full rounded-xl p-2 text-left transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-200 ${active ? "bg-blue-50 ring-1 ring-blue-200" : ""}`}
      title={`Klik voor overzicht: ${label}`}
    >
      {content}
    </button>
  );
...
export default function Dashboard() {
    ...
    async function run() {
      ...
      try {
        ...
        const { data: suppliers, error: supplierErr } = await client
          ...
          .select("id,name,classification,supplier_type,created_at,category,status,is_active,governance_score,checked_items,total_items,notes_count")
        ...
        // Kroonjuwelen mogen niet uit het softwarelandschap of importrecords komen.
        // Definitie dashboard: bestaande Governix-applicatie + kritisch gemarkeerd + functioneel beheer ingericht.
        // Daardoor tellen losse software_landscape_import records en geïmporteerde landschaprecords zonder leverancier niet mee.
        const dashboardCriticalApps = (apps || []).filter((app) => {
          if (!app?.supplier_id) return false;
          if (!app?.is_critical) return false;
          const ownerRecord = assignmentByKey.get(app.id) || assignmentByKey.get(`${app.supplier_id}::${String(app.name || "").toLowerCase()}`);
          return !!ownerRecord;
        });

        for (const app of dashboardCriticalApps) {
          const ownerRecord = assignmentByKey.get(app.id) || assignmentByKey.get(`${app.supplier_id}::${String(app.name || "").toLowerCase()}`);
          ...
          const entry = {
            ...
            domain: app.domain || app.domein || app.category || app.application_domain || "",
            ownerName,
            ownerMeta,
        ...
        if (!cancelled) {
          setRows(list);
          setCriticalApps((dashboardCriticalApps || []).map((app) => {
            const ownerRecord = assignmentByKey.get(app.id) || assignmentByKey.get(`${app.supplier_id}::${String(app.name || "").toLowerCase()}`);
            const ownerName = ownerRecord?.contact_name || app.functional_owner || "Nog niet gekoppeld";
            const ownerMeta = ownerRecord?.email || ownerRecord?.role_title || "";
  ...
  const metricConfig = useMemo(() => {
    const base = {
      ...
      traffic_green: {
        title: "Traffic light: Groen",
        description: "Leveranciers met governance-score van 75% of hoger.",
        rows: rows.filter((row) => governanceToLight(row.governancePercent) === "green"),
      },
      traffic_amber: {
        title: "Traffic light: Oranje",
        description: "Leveranciers met governance-score vanaf 50% tot 75%.",
        rows: rows.filter((row) => governanceToLight(row.governancePercent) === "amber"),
      },
      traffic_red: {
        title: "Traffic light: Rood",
        description: "Leveranciers met governance-score lager dan 50%.",
        rows: rows.filter((row) => governanceToLight(row.governancePercent) === "red"),
      },
    };

    for (const [label] of domainStats) {
      base[`domain:${label}`] = {
        title: `Domein: ${label}`,
        description: `Leveranciers binnen het domein ${label}.`,
        rows: rows.filter((row) => supplierDomainLabel(row.domain) === label),
      };
    }

    for (const [label] of classificationStats) {
      base[`classification:${label}`] = {
        title: `Kraljic-categorie: ${label}`,
        description: `Leveranciers met de classificatie ${label}.`,
        rows: rows.filter((row) => (row.classification || "Onbekend") === label),
      };
    }

    return base;
  }, [rows, currentYear, domainStats, classificationStats]);
    ...
    <div className="space-y-6">
      <section className="rounded-[2rem] bg-[linear-gradient(135deg,#0b3a74_0%,#0f4f9f_50%,#2563eb_100%)] p-6 text-white shadow-2xl md:p-8">
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div className="max-w-5xl presentation-readable">
            ...
            <p className="mt-4 text-white/85 leading-7">Governix brengt governance, leveranciersstrategie en opvolging samen in een visueel dashboard dat direct door te klikken is naar de onderliggende set.</p>
          </div>
      ...
      {usingDemo ? (
        <Notice title="Voorbeelddata actief">
          Er zijn nog geen leveranciers gevonden voor deze organisatie. Daarom toont dit dashboard tijdelijke voorbeelddata.
        </Notice>
      ) : null}
      ...
      <div className="presentation-grid-compact grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-7">
        <MetricCard icon={Building2} label="Leveranciers" value={summary.suppliers} subtext="Totaal in deze organisatie" tone="slate" active={activeMetric === "suppliers"} onClick={() => setActiveMetric("suppliers")} />
        <MetricCard icon={ShieldCheck} label="Gem. governance" value={`${summary.avgGovernance}%`} subtext="Checklistvolwassenheid" tone="blue" active={activeMetric === "healthy"} onClick={() => setActiveMetric("healthy")} />
        <MetricCard icon={AlertTriangle} label="Gem. risico" value={summary.avgRisk || "0.0"} subtext="Gemiddelde risicoscore (1–3)" tone="amber" active={activeMetric === "risk"} onClick={() => setActiveMetric("risk")} />
        <MetricCard icon={Bot} label="AI / subverwerkers" value={summary.aiSuppliers} subtext="Leveranciers met aanvullende verwerking" tone="violet" active={activeMetric === "ai"} onClick={() => setActiveMetric("ai")} />
        <MetricCard icon={Gem} label="Kroonjuwelen" value={summary.criticalAppsCount} subtext={`${summary.criticalSuppliers} leverancier(s) geraakt`} tone="violet" active={activeMetric === "strategic"} onClick={() => setActiveMetric("strategic")} />
        <MetricCard icon={ClipboardCheck} label={`Beoordeeld in ${currentYear}`} value={summary.evaluatedThisYear} subtext="Leveranciers met actuele beoordeling" tone="green" active={activeMetric === "evaluated"} onClick={() => setActiveMetric("evaluated")} />
        <MetricCard icon={CheckSquare} label="Open acties" value={summary.openActions} subtext="Op basis van notities en opvolging" tone="amber" active={activeMetric === "actions"} onClick={() => setActiveMetric("actions")} />
      </div>

      <MetricResultsPanel metric={activeMetricData} rows={activeMetricData?.rows || []} onClose={() => setActiveMetric(null)} />

      <div className="presentation-split grid gap-6 xl:grid-cols-[1.25fr_1fr]">
        <div className="card p-6">
        ...
        <div className="space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="mt-4 space-y-4">
              <BarRow label="Groen" value={summary.green} total={summary.suppliers} tone="bg-emerald-500" active={activeMetric === "traffic_green"} onClick={() => setActiveMetric("traffic_green")} />
              <BarRow label="Oranje" value={summary.amber} total={summary.suppliers} tone="bg-amber-500" active={activeMetric === "traffic_amber"} onClick={() => setActiveMetric("traffic_amber")} />
              <BarRow label="Rood" value={summary.red} total={summary.suppliers} tone="bg-rose-500" active={activeMetric === "traffic_red"} onClick={() => setActiveMetric("traffic_red")} />
            </div>
          </div>
          ...
          <div className="card p-6">
            ...
            <div className="mt-4 space-y-4">
              {domainStats.length ? domainStats.map(([label, value]) => (
                <BarRow key={label} label={label} value={value} total={summary.suppliers} tone="bg-indigo-500" active={activeMetric === `domain:${label}`} onClick={() => setActiveMetric(`domain:${label}`)} />
          ...
          <div className="card p-6">
            ...
            <div className="mt-4 space-y-4">
              {classificationStats.length ? classificationStats.map(([label, value]) => (
                <BarRow key={label} label={label} value={value} total={summary.suppliers} tone="bg-blue-500" active={activeMetric === `classification:${label}`} onClick={() => setActiveMetric(`classification:${label}`)} />
      ...
      <div className="presentation-split grid gap-6 xl:grid-cols-[1.25fr_0.8fr]">
        <div className="card p-6">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
