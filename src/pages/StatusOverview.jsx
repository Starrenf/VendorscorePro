    ...
    async function run() {
      ...
      try {
        ...
        const { data, error } = await client
          ...
          .select("id,name,category,classification,supplier_type,governance_score,checked_items,total_items,notes_count")
        ...
        const entries = list.map((supplier) => ({
          ...
          total: Number(supplier.total_items || 0),
          done: Number(supplier.checked_items || 0),
        }));
    ...
    <div className="space-y-4 rounded-3xl border border-white/70 bg-white/92 p-4 md:p-6 shadow-xl backdrop-blur-sm">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="text-right text-sm text-slate-600">
          ...
          <div>{organization?.name || "Voorbeelddata"}</div>
        </div>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
        <div>
          <div className="text-sm font-semibold text-slate-900">Voorbeelddata</div>
          <div className="text-xs text-slate-600">Schakel tijdelijk tussen voorbeelddata en echte organisatiegegevens.</div>
        </div>
        <DemoModeToggle />
      </div>
      ...
      {usingDemo ? (
        <Notice title="Voorbeelddata actief">
          Deze statuspagina toont tijdelijke voorbeeldleveranciers zolang er nog geen eigen leveranciers beschikbaar zijn.
        </Notice>
      ) : null}
