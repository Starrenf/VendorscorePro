...
export default function AdminWeights() {
  ...
  async function saveAll() {
    ...
    try {
      ...
      try {
        await saveWithToast(
          client.from("weight_configs").upsert(payload, { onConflict: "organization_id,strategy,k_block" }),
          toast,
          {
            loading: "Beoordelingswaardes opslaan...",
            success: "Beoordelingswaardes opgeslagen.",
            error: "Beoordelingswaardes opslaan mislukt.",
          },
        );
        setRows(payload);
      } catch (error) {
        if (/constraint|conflict|unique/i.test(error?.message || "")) {
          await saveWithToast(client.from("weight_configs").delete().eq("organization_id", organization.id), toast, {
            loading: "Bestaande waardes vervangen...",
            success: "Bestaande waardes verwijderd.",
            error: "Bestaande waardes verwijderen mislukt.",
          });
          await saveWithToast(
            client.from("weight_configs").insert(payload.map(({ id, ...rest }) => rest)),
            toast,
            {
              loading: "Beoordelingswaardes opnieuw opslaan...",
              success: "Beoordelingswaardes opgeslagen.",
              error: "Beoordelingswaardes opslaan mislukt.",
            },
          );
      ...
      <div className="card p-6">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <Notice title="Multi-tenant">
            Alleen de regels van de actieve organisatie worden geladen en opgeslagen. Daardoor blijven instellingen netjes gescheiden per organisatie.
          </Notice>
        </div>
      </div>
    </div>
