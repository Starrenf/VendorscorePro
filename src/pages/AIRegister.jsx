...
export default function AIRegister() {
  ...
  async function loadAll() {
    ...
    setLoading(true);
    setErr("");
    const [aiRes, supRes, appRes, mitRes, profRes] = await Promise.all([
      ...
      client.from("profiles").select("id, full_name").eq("organization_id", orgId),
    ]);
  ...
  function edit(row) {
    ...
    // Bewust geen scroll-reset: gebruiker blijft in context.
  }
  ...
  async function save() {
    ...
    const duplicate = rows.find((row) => {
      ...
      return (row.name || "").trim().toLowerCase() === normalizedName
        ...
        && (row.application_id || "") === (form.application_id || "");
    });

    if (duplicate && !window.confirm(`Er bestaat al een AI-registeritem met deze leverancier, applicatie en naam. Wil je het bestaande item bijwerken in plaats van een nieuw item aanmaken?`)) {
      edit(duplicate);
      setErr("Bestaand AI-registeritem geopend. Controleer en sla opnieuw op.");
      return;
    }

    setSaving(true);
    const payload = cleanPayload({ ...form, id: duplicate?.id || form.id }, orgId, session?.user?.id);
    const targetId = form.id || duplicate?.id || null;
    const res = targetId
      ? await client.from("ai_register").update(payload).eq("id", targetId).eq("organization_id", orgId).select("*").maybeSingle()
    ...
    toast.success(targetId ? "AI-registeritem bijgewerkt." : "AI-registeritem opgeslagen.");
    setForm(EMPTY_FORM);
    setSelectedId(null);
    await loadAll();
  }
  ...
  function displayUser(userId) {
    ...
    if (userId === session?.user?.id) return profile?.full_name || session?.user?.email || "Jij";
    return profiles.find((p) => p.id === userId)?.full_name || "Gebruiker";
  }

  const enriched = useMemo(() => rows.map((row) => ({
    ...row,
    supplierName: suppliers.find((s) => s.id === row.supplier_id)?.name || "Niet gekoppeld",
    applicationName: applications.find((a) => a.id === row.application_id)?.name || "Niet gekoppeld",
    createdByName: displayUser(row.created_by),
    warnings: buildAiWarnings(row),
  })), [rows, suppliers, applications, profiles, profile, session]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4 space-y-4">
        <div><div className="font-semibold">{selectedId ? "AI-registeritem bewerken" : "Nieuw AI-registeritem"}</div><p className="mt-1 text-sm text-slate-600">Open een bestaand item via Bewerken om het record bij te werken. VendorScorePro voorkomt dubbele AI-registraties per leverancier, applicatie en naam.</p></div>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <div className="space-y-1 md:col-span-3"><span className="text-sm font-medium">Transparantiemaatregelen / notities</span><RichTextEditor value={form.transparency_measures || ""} onChange={(value) => setField("transparency_measures", value)} placeholder="Bijv. gebruikers informeren, AI-label, instructie, logging, disclaimer." minHeight={160} /></div>
        </div>
