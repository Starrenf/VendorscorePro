...
import SupplierAccountantOverviewTab from "../components/suppliers/SupplierAccountantOverviewTab";
import SupplierRetentionPeriodsTab from "../components/suppliers/SupplierRetentionPeriodsTab";
import SupplierAIRegisterTab from "../components/suppliers/SupplierAIRegisterTab";
import SupplierWorkPackagesTab from "../components/suppliers/SupplierWorkPackagesTab";
import {
...
function Field({ label, required, children, hint }) {
  return (
    <label className="vsp-edit-field">
...
export default function SupplierDetail() {
  ...
  const [govSaveState, setGovSaveState] = useState("clean");
  const [govLastSavedAt, setGovLastSavedAt] = useState(null);
  ...
  const effectiveOrganization = organization || (orgId ? { id: orgId, name: "Gilde Opleidingen" } : null);

  const selectTab = (key) => {
    // UX refresh: wisselen tussen tabs mag de gebruiker niet uit context halen.
    // Eerder sprong de pagina naar boven, waardoor het scherm onrustig voelde.
    setTab(key);
  };
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setDraft({
        ...
        kvk_number: s.kvk_number ?? s.kvk ?? "",
        vat_number: s.vat_number ?? "",
        iban: s.iban ?? "",
        classification: s.classification ?? "",
        supplier_type: s.supplier_type ?? "",
        creditor_number: s.creditor_number ?? "",
        category: s.category ?? "Niet ingevuld",
        domain: s.domain ?? s.category ?? "Generiek",
        visiting_address: s.visiting_address ?? "",
  ...
  const onToggleGov = (itemKey, next) => {
    setGovernance((current) => ({
      checks: { ...(current?.checks || {}), [itemKey]: !!next },
      notes: { ...(current?.notes || {}) },
    }));
    setGovSaveState("dirty");
  };

  const onNoteGov = (itemKey, note) => {
    setGovernance((current) => ({
      checks: { ...(current?.checks || {}) },
      notes: { ...(current?.notes || {}), [itemKey]: note },
    }));
    setGovSaveState("dirty");
  };

  async function saveGovernanceChecklist() {
    if (!client || !orgId || !id) {
      toast.error("Governance kan nog niet worden opgeslagen: organisatie of leverancier ontbreekt.");
      setGovSaveState("error");
      return;
    }

    setGovSaveState("saving");
    try {
      ...
      setGovSaveState("clean");
      setGovLastSavedAt(new Date());
      toast.success("Governance-checklist opgeslagen.");
    } catch (e) {
      console.warn("saveGovernanceChecklist failed", e);
      setGovSaveState("error");
      toast.error(e?.message || "Opslaan van governance-checklist mislukt.");
    }
  }

  useEffect(() => {
    if (govSaveState !== "dirty") return undefined;
  ...
  async function saveProgress() {
    try {
      await saveWithToast(
        () =>
          client.from("supplier_progress").upsert(
            {
        ...
        toast,
        {
          loading: "Voortgang opslaan...",
          success: "Voortgang opgeslagen.",
          error: "Voortgang opslaan mislukt.",
        },
      );
    } catch (err) {
      // toast wordt centraal afgehandeld
    }
  }
  ...
  async function save() {
    ...
    const loadingToastId = toast?.info
      ? toast.info("Leverancier opslaan...", "Bezig", { duration: 0 })
      : null;
    ...
    try {
      const payload = cleanUpdate({
        ...
        supplier_type: (draft.supplier_type || "").trim() || null,
        creditor_number: (draft.creditor_number || "").trim() || null,
        category: (draft.category || "Niet ingevuld").trim() || "Niet ingevuld",
        domain: (draft.domain || draft.category || "Generiek").trim() || "Generiek",
        visiting_address: (draft.visiting_address || "").trim() || null,
        invoice_address: (draft.invoice_address || "").trim() || null,
        postal_code: (draft.postal_code || "").trim() || null,
        city: (draft.city || "").trim() || null,
        country: (draft.country || "Nederland").trim() || "Nederland",
      ...
      setDraft({
        ...
        kvk_number: data.kvk_number ?? data.kvk ?? "",
        vat_number: data.vat_number ?? "",
        iban: data.iban ?? "",
        classification: data.classification ?? "",
        supplier_type: data.supplier_type ?? "",
        creditor_number: data.creditor_number ?? "",
        category: data.category ?? "Niet ingevuld",
        domain: data.domain ?? data.category ?? "Generiek",
        visiting_address: data.visiting_address ?? "",
      ...
      toast?.success?.("Leverancier en contactpersonen opgeslagen.");
    } catch (e) {
      const msg = formatSbError(e);
      if (loadingToastId && toast?.dismiss) toast.dismiss(loadingToastId);
      toast?.error?.(msg || "Leverancier opslaan mislukt.");
      setErr(msg);
    } finally {
      setSaving(false);
    }
  ...
  const tabs = [
    ...
    { key: "subprocessors", label: "Subverwerkers" },
    { key: "work_packages", label: "Onderaannemers" },
    { key: "ai_register", label: "AI-register" },
    ...
    { key: "retention", label: "Bewaartermijnen" },
  ];
      ...
      <div className="card p-0 overflow-hidden">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border border-slate-200 bg-white text-slate-700">
                Domein: {supplierDomainLabel(draft?.domain || draft?.category)}
              </span>
              {lastEval?.strategy ? (
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-600">
              ...
              <span className="badge">
                org: {effectiveOrganization?.name || "Gilde Opleidingen"}
              </span>
            </div>
          </div>
          ...
          <div className="card glass-soft p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Overzicht van inventarisatie, contractstatus en overleg. Deze sectie slaat alleen voortgang op, niet de contractmetadata zelf.
                </p>
              </div>
              <span className="badge">
            ...
            <div className="mt-3 rounded-xl bg-white/60 p-3 text-sm text-slate-700">
              ...
              <div className="mt-1 text-xs text-slate-500">
                Let op: deze sectie bewaart alleen voortgangsvelden. Contractgegevens zelf beheer je in het tabblad "Contracten".
              </div>
              <textarea
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <button className="btn btn-primary" type="button" onClick={saveProgress}>
                  Voortgang opslaan
                </button>
                <button className="btn" type="button" onClick={() => selectTab("contracten")}>
                  Naar contracten
                </button>
                <button className="btn" type="button" onClick={() => selectTab("governance")}>
                  Naar checklists
                </button>
              </div>
            </div>
        ...
        <div className="grid md:grid-cols-[240px_1fr]">
          ...
          <div className="p-5">
            ...
            {tab === "gegevens" ? (
              <div className="vsp-view-grid">
                <div className="vsp-section">
                  <SectionHeader title="Leveranciersgegevens" subtitle="Basisgegevens worden bewust gegroepeerd zodat bewerken overzichtelijk blijft." />
                  ...
                  <Field label="Juridische naam" hint="Officiële handels-/statutaire naam zoals KvK of contractpartij.">
                    <input
                      value={draft.legal_name || ""}
                      onChange={(e) => setField("legal_name", e.target.value)}
                      className="w-full"
                      placeholder="Bijv. BINX Smartility B.V."
                    />
                  </Field>
                  ...
                  <Field label="IBAN">
                    <input
                      ...
                      className="w-full"
                      placeholder="Bijv. NL00BANK0123456789"
                    />
                  </Field>
                  ...
                  <Field label="Leverancierstype">
                    <input
                      value={draft.supplier_type || ""}
                      onChange={(e) => setField("supplier_type", e.target.value)}
                      className="w-full"
                      placeholder="Bijv. Hoofdaannemer, Onderaannemer, Softwareleverancier"
                    />
                  </Field>
                  ...
                  <Field label="Discipline / domein">
                    <select
                      value={draft.domain || "Generiek"}
                  ...
                  <Field label="Categorie / contractdiscipline">
                    <input
                      value={draft.category || ""}
                      onChange={(e) => setField("category", e.target.value)}
                      className="w-full"
                      placeholder="Bijv. Liftinstallatie, Catering, SaaS, Integraal onderhoud"
                    />
                  </Field>

                  <Field label="Bezoekadres">
                    <input value={draft.visiting_address || ""} onChange={(e) => setField("visiting_address", e.target.value)} className="w-full" />
                  </Field>

                  <Field label="Factuuradres">
                    <input value={draft.invoice_address || ""} onChange={(e) => setField("invoice_address", e.target.value)} className="w-full" />
                  </Field>

                  <Field label="Postcode">
                    <input value={draft.postal_code || ""} onChange={(e) => setField("postal_code", e.target.value)} className="w-full" />
                  </Field>

                  <Field label="Plaats">
                    <input value={draft.city || ""} onChange={(e) => setField("city", e.target.value)} className="w-full" />
                  </Field>

                  <Field label="Land">
                    <input value={draft.country || "Nederland"} onChange={(e) => setField("country", e.target.value)} className="w-full" />
                  </Field>

                  <Field label="Website">
                    <input value={draft.website || ""} onChange={(e) => setField("website", e.target.value)} className="w-full" placeholder="https://..." />
                  </Field>

                  <Field label="Algemene e-mail">
                    <input value={draft.contact_email || ""} onChange={(e) => setField("contact_email", e.target.value)} className="w-full" placeholder="info@..." />
                  </Field>

                  <Field label="Telefoon">
                    <input value={draft.contact_phone || ""} onChange={(e) => setField("contact_phone", e.target.value)} className="w-full" />
                  </Field>

                  <Field label="Verrijkingsstatus">
                    <select value={draft.enrichment_status || ""} onChange={(e) => setField("enrichment_status", e.target.value)} className="w-full">
                      <option value="">— kies —</option>
                      <option value="Verrijkt">Verrijkt</option>
                      <option value="Verrijkt - controleren">Verrijkt - controleren</option>
                      <option value="Verrijkt - entiteit controleren">Verrijkt - entiteit controleren</option>
                      <option value="Te verifiëren">Te verifiëren</option>
                    </select>
                  </Field>

                  <Field label="Verrijkingsnotities">
                    <textarea value={draft.enrichment_notes || ""} onChange={(e) => setField("enrichment_notes", e.target.value)} className="w-full min-h-[90px]" />
                  </Field>
                  ...
                  <div className="vsp-sticky-save md:col-span-2">
                    <div>
                      <div className="font-extrabold">Wijzigingen leverancier</div>
                      <div className="text-sm text-blue-800/75">Opslaan zonder de pagina te verlaten.</div>
                    </div>
                ...
                <aside className="vsp-status-rail">
                  <div className="vsp-status-card">
                    <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Leverancier</div>
                    <div className="mt-1 text-lg font-extrabold text-slate-950">{draft.name || supplier?.name || "-"}</div>
                  </div>
                  <div className="vsp-status-card">
                    <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Classificatie</div>
                    <div className="mt-1 font-bold text-slate-900">{draft.classification || "Nog niet ingevuld"}</div>
                  </div>
                  <div className="vsp-status-card">
                    <div className="text-xs font-bold uppercase tracking-wide text-slate-500">Domein</div>
                    <div className="mt-1 font-bold text-slate-900">{supplierDomainLabel(draft.domain || draft.category || "Generiek")}</div>
                  </div>
                </aside>
              </div>
            ) : null}
            ...
            {tab === "applications" ? (
              <SupplierApplicationsTab supplier={supplier} organization={effectiveOrganization} />
            ) : null}
            ...
            {tab === "work_packages" ? (
              <SupplierWorkPackagesTab supplier={supplier} organization={effectiveOrganization} />
            ) : null}
              ...
              <div className="max-w-4xl">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  ...
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${governanceStatusClass}`}>
                      {governanceStatusLabel}
                    </span>
                    {govLastSavedAt ? (
                      <span className="text-xs text-slate-500">
                        Laatst opgeslagen: {govLastSavedAt.toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit" })}
                      </span>
                    ) : null}
                    <button className="btn btn-primary" disabled={govSaveState === "saving" || govLoading} onClick={saveGovernanceChecklist}>
                      {govSaveState === "saving" ? "Opslaan…" : "Opslaan"}
                    </button>
                ...
                {govLoading ? (
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    <div className="sticky bottom-4 z-10 rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-lg backdrop-blur flex items-center justify-between gap-3 flex-wrap">
                      <div>
                        <div className="text-sm font-semibold text-slate-900">Governance checklist</div>
                        <div className="text-xs text-slate-600">{governanceStatusLabel}</div>
                      </div>
                      <button className="btn btn-primary" disabled={govSaveState === "saving" || govLoading} onClick={saveGovernanceChecklist}>
                        {govSaveState === "saving" ? "Opslaan…" : "Opslaan governance"}
                      </button>
                    </div>
                  </div>
                )}
            ...
            {tab === "accountant" ? (
              <SupplierAccountantOverviewTab
                supplier={supplier}
                organization={effectiveOrganization}
              />
            ) : null}
            {tab === "ai_register" ? (
              <SupplierAIRegisterTab
                supplier={supplier}
                organization={effectiveOrganization}
              />
            ) : null}
            {tab === "retention" ? (
              <SupplierRetentionPeriodsTab
                supplier={supplier}
                organization={effectiveOrganization}
              />
            ) : null}
