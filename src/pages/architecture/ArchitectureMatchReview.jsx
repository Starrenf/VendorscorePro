...
export default function ArchitectureMatchReview() {
  ...
  const rows = useMemo(() => {
    ...
    return list.filter((row) => {
      ...
      const queryOk = !q || [row.mora_application_name, row.governix_application_name, row.match_type, row.match_status]
        .join(" ")
        .toLowerCase()
        .includes(q);
  ...
  async function upsertMatch(row, status, applicationId = row.application_id) {
    ...
    try {
      ...
      const payload = {
        ...
        notes: status === "manual"
          ? `Handmatig gekoppeld vanuit reviewflow. Originele kandidaat: ${row.governix_application_name || "onbekend"}.`
          : null,
      };
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-blue-100/85">
            Controleer, bevestig of wijs voorgestelde koppelingen tussen MORA-componenten en Governix-applicaties af.
            ...
            return (
              <div key={`${row.mora_component_id}-${row.application_id}`} className="match-review-card">
                <div className="match-review-main">
                  ...
                  <div>
                    <div className="match-review-label">Governix applicatie</div>
                    <div className="match-review-name">{row.governix_application_name || "—"}</div>
                ...
                <div className="match-explainability">
                  ...
                  <div className="match-explainability-grid">
                    <div className="match-factor">
                      ...
                      <strong>{row.mora_application_name || "—"} ↔ {row.governix_application_name || "—"}</strong>
                    ...
                    <div className="match-factor">
                      ...
                      <small>Governix domein: {appById.get(row.application_id)?.domain || "niet ingevuld"}</small>
                    </div>
