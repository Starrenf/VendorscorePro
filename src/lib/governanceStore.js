...
export async function saveGovernance({ client, organizationId, supplierId, payload }) {
  const normalized = normalizePayload(payload);
  writeLocal(organizationId, supplierId, normalized);
  ...
  const updatedBy = await getCurrentUserId(client);
  const keys = new Set([
    ...Object.keys(normalized.checks || {}),
    ...Object.keys(normalized.notes || {}),
  ]);
  ...
  const rows = [...keys].map((key) =>
    buildChecklistRow({
      ...
      supplierId,
      key,
      checked: !!normalized.checks?.[key],
      noteText: normalized.notes?.[key] || null,
      updatedBy,
    }),
  );
  ...
  if (rows.length) {
    const { error } = await client
      .from("governance_checklist_items")
      .upsert(rows, { onConflict: "organization_id,supplier_id,item_key" });
